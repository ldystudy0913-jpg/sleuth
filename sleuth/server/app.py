...
from .envelope import app_error_handler, json_ok, raise_code
...
def _check_admin(request, config) -> None:
...
def _message_file_ids(message, config) -> list:
    key = str(getattr(getattr(config, "files", None), "message_file_ids_key", "") or "file_ids")
    raw = (getattr(message, "metadata", None) or {}).get(key)
    if not isinstance(raw, list):
        return []
    return [str(x).strip() for x in raw if str(x).strip()]


def _session_messages_payload(rec, messages, *, config, scrub_on: bool, priv) -> list:
    from ..files.mailbox import public_files
    from ..privacy import maybe_desensitize

    rows = []
    for m in messages:
        ids = _message_file_ids(m, config)
        rows.append(
            {
                ...
                **message_timing_fields(m.metadata),
                "files": public_files(rec, file_ids=ids, config=config) if ids else [],
            }
        )
    return rows


def _cos_app_error(exc, item: BizErrorCode) -> APPError:
    return APPError.of(item, str(exc), status=503)
...
def create_app(workdir: Optional[Path] = None):
    ...
    async def create_session(request: Request):
        ...
        if err:
            raise_code(BizErrorCode.AGENT_CONFIG_INVALID, err)
        from ..memory.acl import session_acl_error

        acl_err = session_acl_error(sess)
        if acl_err:
            raise_code(BizErrorCode.AUTH_NOT_PERMIT, acl_err)
    ...
    async def get_session(request: Request):
        ...
        if rec is None or (rec.user_id and rec.user_id != user_id):
            raise_code(BizErrorCode.SESSION_NOT_FOUND, sid, status=404)
        ...
        return json_ok(
            {
                ...
                "files": public_files(rec, config=config),
                "messages": _session_messages_payload(
                    rec, messages, config=config, scrub_on=scrub_on, priv=priv
                ),
            }
        )
    ...
    def _bind_prompt_files(sess, body: Dict[str, Any]) -> None:
        ...
        if raw is None:
            ...
            return
        if not isinstance(raw, list):
            raise_code(BizErrorCode.REQUEST_VALIDATION_FAILED, "file_ids must be an array")
        ...
        if unknown:
            raise_code(BizErrorCode.PARAM_INVALID, f"unknown file_ids: {', '.join(unknown)}")
        not_ready = [
            fid
            for fid in ids
        ...
        if not_ready:
            raise_code(BizErrorCode.FILE_NOT_READY, ", ".join(not_ready))
        sess._prompt_file_ids = ids

    def _turn_app_error(turn_err: str) -> None:
        low = (turn_err or "").lower()
        if "mcp" in low:
            raise_code(BizErrorCode.MCP_CALL_FAILED, turn_err)
        raise_code(BizErrorCode.TURN_FAILED, turn_err)
    ...
    def _run_turn(sess, body: Dict[str, Any], prompt: str, *, run_host: bool = True):
    ...
    async def post_message(request: Request):
        ...
        if err:
            raise_code(BizErrorCode.REQUEST_VALIDATION_FAILED, err)
    ...
    async def post_message_stream(request: Request):
        ...
        text, turn_err, turn = _run_turn(
            sess, body, str(prompt or ""), run_host=False
        )
        if turn_err:
            _turn_app_error(turn_err)
        ...
        async def event_gen():
            ...
            try:
                ...
                done["code"] = BizErrorCode.SUC0000.code
                done["msg"] = BizErrorCode.SUC0000.error_message
                yield sse_pack(done)
    ...
    async def user_usage(request: Request):
        ...
        if header_user != user_id and header_user != "anonymous":
            _check_admin(request, config)
        return json_ok(store.sum_usage(user_id))
    ...
    async def models_list(_: Request):
        return json_ok(models_payload(load(workdir)))
    ...
    async def mcp_status(_: Request):
        return json_ok(mcp_status_dict(load(workdir)))

    async def mcp_reload(request: Request):
        _check_admin(request, config)
        return json_ok(reload_mcp(load(workdir), workdir))
    ...
    async def deprecated_presign_upload(request: Request):
        ...
        raise_code(BizErrorCode.API_DEPRECATED, msg, status=410)

    async def upload_session_file(request: Request):
        rec = _owned_rec(request)
        cfg = load(workdir)
        ...
        try:
            ...
            raise _cos_app_error(exc, BizErrorCode.UPLOAD_S3_FAIL) from exc
        except MailboxError:
            raise
        rec = store.get_session(rec.id) or rec
        view = public_file(rec, item, include_pending=True, config=cfg)
        return json_ok(view or {"id": item.get("id"), "status": item.get("status")})
    ...
    async def download_session_file(request: Request):
        ...
        try:
            ...
            raise _cos_app_error(exc, BizErrorCode.DOWNLOAD_S3_FAIL) from exc
        except MailboxError:
            raise
    ...
    async def delete_session_file(request: Request):
        ...
        try:
            ...
            raise _cos_app_error(exc, BizErrorCode.DELETE_S3_FAIL) from exc
        except MailboxError:
            raise
        return json_ok({"ok": True, "id": file_id})

    async def files_limits(request: Request):
        from ..files.limits import files_limits_payload

        return json_ok(files_limits_payload(load(workdir)))
    ...
    async def get_orchestration_job(request: Request):
        ...
        if job is None:
            raise_code(BizErrorCode.JOB_NOT_EXIST, job_id, status=404)
        return json_ok(job)

    routes = [
        Route("/health", health),
        Route("/v1/files/limits", files_limits, methods=["GET"]),
