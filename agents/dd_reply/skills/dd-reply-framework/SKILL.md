...
4. 向用户展示返回的 `markdown`（或四段字段）；保留全部【待核实N】槽位，不要替客户经理填最终结论。第 4 段须是「核实结果 → 可排除 / 可缓释 / 无法排除」对照，免责声明不能替代该段。知识链接在工具 JSON 顶层 `sources[]`（`title` + `url`）；Sleuth 会在最终答复末尾列出，不必按某种 markdown 标题手抄，也不要截断或改写 URL。
