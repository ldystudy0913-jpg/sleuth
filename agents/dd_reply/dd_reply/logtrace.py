...
def _require_fastapi_log_trace():
    try:
    ...
    return FastapiLogTrace


def _bind_official_tracer(path: Optional[str], verbose: bool, async_mode: bool) -> Any:
    """Construct FastapiLogTrace then bind the official module singleton."""
    import log_trace

    FastapiLogTrace = _require_fastapi_log_trace()
    if path:
        FastapiLogTrace(path, verbose=verbose, async_mode=async_mode)
    else:
        FastapiLogTrace(verbose=verbose, async_mode=async_mode)
    getter = getattr(log_trace, "get_tracer", None)
    if not callable(getter):
        raise RuntimeError("log_trace.get_tracer is missing; upgrade the intranet package")
    tracer = getter()
    try:
        import log_trace.http_client  # noqa: F401
    except ImportError:
        pass
    return tracer
...
def ensure_initialized(config: Any = None, *, workdir: Optional[Path] = None) -> None:
    ...
    _tracer = _bind_official_tracer(path, _verbose(config), _async_mode(config))
...
def install_mcp_middleware(server: Any, config: Any = None) -> None:
    """为 FastMCP server 安装 log_trace 中间件。

    log_trace 5.1.1+ 不再需要单独的 FastMcpLogTraceMiddleware，
    直接复用 FastapiLogTraceMiddleware 即可（FastMCP 底层也是 starlette）。
    """
    if not is_enabled(config):
        return
    ensure_initialized(config)
...
def traced_httpx():
    """httpx with official Client.send patch when log_trace.http_client is importable."""
    try:
        from log_trace.http_client import httpx
    except ImportError:
        import httpx
    return httpx
